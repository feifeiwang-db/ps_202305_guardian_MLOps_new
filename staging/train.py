...
df_train0 = spark.createDataFrame(df)
df_train0 = df_train0.select(
    '*',
...
df_train = df_train0.select(* (feature_cols+[y_col])).withColumnRenamed(y_col, "y")
display(df_train)
...
pd_train = df_train.toPandas().sort_values(by=['Daily_ICU_occupancy', 'Daily_hospital_occupancy', 'quarter'])
...
with mlflow.start_run(run_name='my_run_xgboost') as run:
  ...
  X_train_with_y_train = X_train.copy() #to be logged using MLflow, later for monitoring purpose
  X_train_with_y_train['y'] = y_train

  run_id = mlflow.search_runs(filter_string='tags.mlflow.runName = "my_run_xgboost"').iloc[0].run_id
  mlflow.xgboost.log_model(registered_model_name=model_name, xgb_model=model, conda_env=conda_env, signature=signature, input_example=X_train_with_y_train, artifact_path=f"runs:/{run_id}/{model_name}") 
